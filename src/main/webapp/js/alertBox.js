...
function alertBox(type, msg, showTime, callBack) {
    ...
    if ($("#divAlertBox").length !== 0) {
        ...
        divAlertBox.removeClass().addClass(divCss).slideDown(slidedownTime);
        box(divAlertBox);
    } else {
        ...
        divAlertBox.slideDown(slidedownTime);
        box(divAlertBox);
    }
    setTimeout(function () {
        divAlertBox.fadeOut(slidedownTime);
        if (callBack != null) {
            callBack();
        }
    }, showTime);
...
function confirmBox(type, msg, callBack) {
    ...
    if ($("#divConfirmBox").length !== 0) {
        ...
        divConfirmBox.removeClass().addClass(divCss).slideDown(slidedownTime);
        ...
        divConfirmBox.slideDown(slidedownTime);
        box(divConfirmBox);
    }
}
